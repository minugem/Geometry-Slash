...
void InputSystem::ProcessInput()
{
	...
	m_mousePosition.Set(static_cast<float>(mouseX), static_cast<float>(mouseY)); for (int k = 0; k < m_iNumAttachedControllers; ++k)
	{
	...
	while (SDL_PollEvent(&event) != 0)
	{
		...
		ProcessMouseWheel(event); if (event.type == SDL_QUIT)
		{

...
bool Renderer::Initialise(bool windowed, int width, int height)
{
	...
	if (initialised)
	{
		m_pTextureManager = new TextureManager(); assert(m_pTextureManager);
...
bool Renderer::InitialiseOpenGL(int screenWidth, int screenHeight)
{
	...
	m_glContext = SDL_GL_CreateContext(m_pWindow); GLenum glewResult = glewInit();
	...
	bool shadersLoaded = SetupSpriteShader(); return shadersLoaded;
}
...
void Renderer::CreateStaticText(const char* pText, int pointsize)
{
	...
	pTexture->LoadTextTexture(pText, "..\\assets\\proxon.ttf", pointsize);
